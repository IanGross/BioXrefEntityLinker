...
import random
import json
#from amrCustomReader.Node import *
#from amrCustomReader.Sentence import *

"""
path = "/Users/shruthichari/Documents/MS/Courses/NLP/Assignments/ELTask/craft-2.0/genia-xml/term"
all_subdirs = os.listdir(path)

ground_truths = open("sentences_truths.pkl","wb")
#Will be used for the indexing
unique_truths = open("unique_truths.pkl","wb")
sentenceleveltruths = []
utruthlist = []
	...
	if sub_dir != "sections-and-typography":
		...
		for ind_file in files_subdir:
			...
			if ind_file.endswith("txt.xml"):
				with open(sub_dirpath + "/" + ind_file,"r") as ind_fileo:
					for line in ind_fileo:
						line = line.strip()
						#Non-empty line
						if line != "":
							#To extract sentences from para
							ind_lines = line.split(". ")
							#If any annotations in the sentence
							for ind_line in ind_lines:
								if "term" in ind_line:
									bsOut = BeautifulSoup(ind_line)
									...
									for i in range(len(term_mentions)):
										...
										sent_groundtemplate["word_truths"].append({"word":term_mentions[i].text,"truth":ontology_mapping})
									#Will need to add an AMR portion here
									sent_groundtemplate["sentence"] =  re.sub('<.*?>', '', ind_line)
									sentenceleveltruths.append(sent_groundtemplate)
				print("Finished adding truths for ",ind_file," in ",sub_dir)


pickle.dump(sentenceleveltruths, ground_truths)
ground_truths.close()
